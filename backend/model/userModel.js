...
exports.findByNickname = async ({ nickname }) => {
  ...
  const sql = 'SELECT email FROM user_table WHERE nickname = ? AND deleted_at IS NULL';
  const result = await query(sql, [cleanNickname]);

  console.log('[MODEL] result:', result); 
  return result?.[0] || null;
};

exports.findByEmailAndNickname = async ({ email, nickname }) => {
  const sql = `
    SELECT user_id, email, nickname
    FROM user_table
    WHERE email = ? AND nickname = ? AND deleted_at IS NULL;
  `;
